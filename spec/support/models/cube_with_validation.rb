...
class CubeWithValidation < ActiveRecord::Base
  ...
  measured_length :length_unit_singular
  validates :length_unit_singular, measured: {units: "ft", message: "custom message"}

  measured_length :length_units_multiple
  validates :length_units_multiple, measured: {units: [:meter, "cm"]}
  ...
  def low_bound
    UnitMeasurements::Length.new(10, :in)
  end

  def high_bound
    UnitMeasurements::Length.new(20, :in)
  end
end
